document.addEventListener('DOMContentLoaded', () => {
     ...
     */
    const loadDocument = (uid, loadUrl) => {
        console.log('Loading document UID:', uid);

        const url = new URL(loadUrl, window.location.origin);
        url.searchParams.set('documentUid', uid);
    ...
    document.addEventListener('click', (e) => {
        ...
        if (item) {
            e.preventDefault();
            loadDocument(item.dataset.uid, item.dataset.loadUrl);
        }
    });
});